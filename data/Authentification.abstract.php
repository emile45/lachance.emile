...
abstract class Authentification {
    ...
    {
        try {
            $this->pdo = $this->connexion->prepare("select pwd from utilisateurs where user=:user");
    ...
    {
        try {
            $this->pdo = $this->connexion->prepare("select count(user) from utilisateurs where user=:user");
            $this->pdo->bindParam(":user",$user,PDO::PARAM_STR,255);
